...
> 版本: v3.0 | 日期: 2026-06-08 | 状态: 设计阶段（小象超市类 · 前置仓配送版）
>
...
| **作业端** | 微信小程序（1 个） | 入库 + 分拣 + 配送，按角色展示不同工作台 |
...
| **入库员** | 邻选·作业 | 采购到货验收、扫码入库、库存调整 |
| **分拣员** | 邻选·作业 | 按订单拣货、复核、打包 |
| **配送员** | 邻选·作业 | 抢单/接单、取货、送达、签收 |
| **仓库主管** | 邻选·作业 + 管理后台 | 波次调度、异常处理、库存盘点 |
...
│ 邻选·作业    Taro 3 + React + TypeScript  (新建)      │
...
│       ├── track/             # 【新增】配送追踪时间轴
│       └── profile/           # 个人中心
├── worker-mini-program/       # 邻选·作业（仓配）— 新建
│   └── src/pages/
│       ├── home/              # 角色工作台入口
│       ├── inbound/           # 入库
│       ├── pick/              # 分拣
...
| v3.0 | 2026-06-08 | **战略转型**：小象超市类前置仓即时配送；新增作业小程序与仓配流程；弱化分销为二期可选 |
